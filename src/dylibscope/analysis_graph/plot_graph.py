...
class Graph:
    ...
    def build_figure(self):
        ...
        self.fig.update_traces(hovertemplate="<b>%{fullData.name}</b><br>Value: %{y}<extra></extra>")
    ...
    def handle_convergence_points(self):
        for metric_key, label in self.metrics.items():
            ...
            dup = tmp.groupby([IOS_VERSION, "value"], observed=False)[self.library_label].agg(list).reset_index()
    ...
    def create_dropdown_buttons(self):
        ...
        for i, label in enumerate(self.metrics.values()):
    ...
    def update_layout(self):
        ...
        self.fig.update_xaxes(type="category", categoryorder="array", categoryarray=VERSION_ORDER)
